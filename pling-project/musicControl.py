...
class musicControl:
  ...
  def playSong(self):
    if globals.pling_type == "dame":
      ...
      self.player = vlc.MediaPlayer("/home/dgmusic/dgmusic/dg_pling/pling-project/files/dame.mp3")
    elif globals.pling_type == "fernet":
	#if int(time.time()) % 2 == 1:
      #self.player = vlc.MediaPlayer(os.path.join(self.script_dir,"files/Track1.mp3"))
      	#else:
      self.player = vlc.MediaPlayer("/home/dgmusic/dgmusic/dg_pling/pling-project/files/fernet.mp3")
    self.player.play()
    time.sleep(0.5)
    #self.findVlcSinkInput() Del av pling for å styre volum med knapper ellerno, men skaper bare trøbbel så ha det bra -hilsen Trym
  ...
  def startPling(self):
    ...
    try:
      ...
      raise (ValueError)
      #raise self.exc_info[1].with.traceback(self.exc_info[2])
